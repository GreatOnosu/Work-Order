<!DOCTYPE html>
<html>
<head runat="server">
	<base href="/Vicky/public/">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>BENSON IDAHOSA UNIVERSITY WORK ORDER SYSTEM</title>
	<link rel="stylesheet" type="text/css" href="css/magic.css">
</head>
<body>
	<header id="adminHeader">
		<div class="header-title">
			<h1>Dashboard</h1>
		</div>
		<div class="user-profile">
			<span><?=$_SESSION['session_username']?></span><a href="logout"><img src="icons/logout.png" title="Logout" /></a>
		</div>
	</header>
	<nav id="breadcrumb">
		<div class="side-menu">
			<img src="icons/menu.png" />
		</div>
		<div class="crumbs">
			<span><a href="Admin/index">Dashboard</a> / </span><span><a href="Admin/fulfilled">Fulfilled Orders</a> / </span><span><a href="Admin/fulfillmain">Maintenance</a></span>
		</div>
	</nav>
	<section>
		<div class="admin-body">
			<div class="body-info">
				<h1 style="color: #381d40;">Maintenance</h1>
				<h3 style="color: #4CAF50; text-align: center;"><?=$stat?></h3>
				<table>
					<tr>
						<th>Title</th>
						<th>Description</th>
						<th>Staff Allocated</th>
						<th>Status</th>
						<th>Time of Assignment</th>
						<th>Time of Completion</th>
						<th>Close Order</th>
				  	</tr>
				  	<?php foreach($records as $record):?>
				  	<tr>
				  		<td><?=$record->title?></td>
				  		<td><?=$record->description?></td>
				  		<td><?=$record->staff_allocated?></td>
				  		<td><?=$record->status?></td>
				  		<td><?=$record->post_time?></td>
				  		<td><?=$record->complete_time?></td>
				  		<td>
				  			<form action="Admin/fulfillmain" method="post">
					  			<input type="hidden" class="btn-login" name="close_id" value="<?=$record->id?>" />
								<input type="submit" class="btn-login" name="close_main" value="Close" />
							</form>
				  		</td>
				  	</tr>
				  	<?php endforeach;?>
				</table>
			</div>
		</div>
	</section>
	<div style="clear: both;"></div>
	<footer id="adminFooter">
		<div>
			<h3>&copy;Benson Idahosa University</h3>
		</div>
	</footer>
</body>
</html>